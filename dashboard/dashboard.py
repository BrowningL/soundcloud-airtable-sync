...
import requests  # <— used for Airtable
...
# Airtable (for Catalogue Size chart)
AIRTABLE_BASE_ID = os.getenv("AIRTABLE_BASE_ID")
AIRTABLE_API_KEY = os.getenv("AIRTABLE_API_KEY")
AIRTABLE_CATALOGUE_TABLE = os.getenv("AIRTABLE_CATALOGUE_TABLE", "Catalogue")
AIRTABLE_RELEASE_FIELD = os.getenv("AIRTABLE_RELEASE_FIELD", "Release Date")
...
<body>
  ...
  <div class="content max-w-7xl mx-auto">
    ...
    <!-- NEW Row 2.5: Catalogue Size Over Time -->
...
<script>
  ...
  // NEW: Catalogue Size Over Time
  async function loadCatalogueChart(days) {
    ...
    catalogueChart = new Chart(ctx, {
      type: 'line',
      data: { labels: data.labels, datasets: [{ label: 'Total Releases (cumulative)', data: data.values, tension: 0.25, fill: false }] },
  ...
  document.getElementById('catalogueDays').addEventListener('change', e => loadCatalogueChart(e.target.value));
  ...
  (async () => {
    ...
    await loadCatalogueChart(document.getElementById('catalogueDays').value);
...
# ── API: catalogue (Airtable-based) ───────────────────────────────────────────
def _airtable_release_dates() -> list[date]:
    """Fetch all Release Date values from Airtable Catalogue."""
    if not AIRTABLE_BASE_ID or not AIRTABLE_API_KEY:
        return []  # keep UI alive even if not configured

    headers = {"Authorization": f"Bearer {AIRTABLE_API_KEY}"}
    params = {
        "pageSize": 100,
        "fields[]": [AIRTABLE_RELEASE_FIELD],
        "sort[0][field]": AIRTABLE_RELEASE_FIELD,
        "sort[0][direction]": "asc",
    }
    url = f"{AIRTABLE_API_URL}/{AIRTABLE_BASE_ID}/{requests.utils.quote(AIRTABLE_CATALOGUE_TABLE)}"
    out: list[date] = []
    offset = None
    while True:
        q = params.copy()
        ...
        for rec in data.get("records", []):
            v = rec.get("fields", {}).get(AIRTABLE_RELEASE_FIELD)
            if not v:
                continue
            try:
                # Airtable date field -> "YYYY-MM-DD" (or ISO with time if field is datetime)
...
def api_catalogue_size_over_time():
    ...
    dates = _airtable_release_dates()
    if not dates:
        # Graceful empty series
        end = date.today(); start = end - timedelta(days=days)
        labels = [d.isoformat() for d in _daterange(start, end)]
        return jsonify({"labels": labels, "values": [0]*len(labels)})

    end = date.today()
    start = end - timedelta(days=days)

    # Baseline count before window start for true cumulative line
    baseline = sum(1 for d in dates if d < start)

    # Count releases per day within window
    per_day = {}
    for d in dates:
        if start <= d <= end:
            per_day[d] = per_day.get(d, 0) + 1

    # Build cumulative daily tuples and fill the range
    running = baseline
    ...
    # Ensure we output a flat line if no releases in window
    if not series:
        series.append((start, baseline))
