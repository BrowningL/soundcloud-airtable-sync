...
# ==========
# HARD CODED CONFIG  (adjust names only if your base/fields change)
# ==========
# Airtable
...
PLAYLISTS_VIEW       = None                 # set a view name (string) to restrict, or keep None
...
# Spotify (client credentials — works for PUBLIC playlists)
SPOTIFY_CLIENT_ID     = "0fe3aee6708647338f3fec1673558a8c"
SPOTIFY_CLIENT_SECRET = "eb12c63ff8a24524af6c5aba83acfb9a"

# QoL / pacing
AIRTABLE_SLEEP_BETWEEN_WRITES = 0.2   # seconds
SPOTIFY_SLEEP_BETWEEN_CALLS   = 0.15  # seconds
MAX_RETRIES                   = 3

SERVICE_NAME = "playlist-followers"
...
def normalize_playlist_id(val: str) -> Optional[str]:
    ...
    Returns the bare ID or None if invalid.
    """
    if not val:
        return None
    v = val.strip()
...
def airtable_find_today_row(playlist_rec_id: str, today_iso: str) -> Optional[str]:
    ...
    Uses IS_SAME so repeats in the same day overwrite instead of duplicating.
    """
    url = _at_url(FOLLOWERS_TABLE)
    # Note the doubled braces to emit Airtable formula braces in an f-string.
    formula = (
...
def airtable_upsert_followers(playlist_rec_id: str, date_iso: str, followers: int) -> None:
    ...
    If a row for today's date exists for this playlist, we PATCH it; otherwise we POST a new row.
    """
    url = _at_url(FOLLOWERS_TABLE)
...
def get_playlist_followers(playlist_id: str, bearer: str) -> Optional[int]:
    """
    Return followers.total for a PUBLIC playlist; None if not accessible (private/invalid).
    """
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        if r.status_code in (401, 403, 404):
            # Likely private/unavailable/invalid ID
            try:
                body = r.json()
            except Exception:
                body = r.text
            print(f"[debug] {playlist_id}: {r.status_code} -> {body}", flush=True)
            return None
...
def sync_once() -> Tuple[int, int]:
    ...
    # De-dupe by playlist_id (keep first link)
    seen = set()
    ...
    print(f"Found {len(order)} playlists in Airtable.", flush=True)
    ...
    for pid, rec_id in order:
        ...
        if count is None:
            print(f"[skip] spotify:playlist:{pid}: could not fetch followers (private/unavailable)", flush=True)
            failed += 1
            continue
        try:
            airtable_upsert_followers(rec_id, today_iso, count)
            print(f"[ok] spotify:playlist:{pid}: {count}", flush=True)
            ok += 1
        except Exception as e:
            print(f"[airtable error] {pid}: {e}", flush=True)
            failed += 1

    print(f"Done. Upserted OK={ok}, failed={failed}.", flush=True)
    return ok, failed
...
@app.get("/")
def root():
    return {"ok": True, "service": SERVICE_NAME}, 200
...
def run_now():
    print(f"[/run] {SERVICE_NAME} starting sync...", flush=True)
    ok, failed = sync_once()
    print(f"[/run] {SERVICE_NAME} finished ok={ok} failed={failed}", flush=True)
    return jsonify({"ok": ok, "failed": failed, "service": SERVICE_NAME}), 200

# Quick debug endpoint: test a single playlist ID/URL/URI from your browser.
@app.get("/debug")
def debug_one():
    ...
    try:
        bearer = get_spotify_app_token()
        count = get_playlist_followers(pid, bearer)
        return jsonify({"id": pid, "followers": count}), 200
    except Exception as e:
